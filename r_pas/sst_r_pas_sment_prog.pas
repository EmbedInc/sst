...
otherwise
    syn_error (str_prog_h, 'sst_pas_read', 'prog_not_allowed_here', nil, 0);
    end;
  nest_level := nest_level + 1;        {down into this PROGRAM block}
  top_block := top_block_prog_k;       {indicate top block is a PROGRAM}

  syn_level_down;                      {down into PROGRAM_STATEMENT syntax level}
  syn_get_tag_msg (                    {get tag for program name}
  ...
  syn_get_tag_msg (                    {get next tag in PROGRAM_STATEMENT}
    tag, str_h, 'sst_pas_read', 'statement_prog_bad', nil, 0);
  if tag <> syn_tag_end_k then begin   {there should be no tag here}
    syn_error_tag_unexp (tag, str_h);
    end;
  syn_level_up;                        {back up from PROGRAM_STATEMENT syntax}
  end;
