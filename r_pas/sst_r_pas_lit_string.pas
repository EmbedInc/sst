...
begin
  ...
  syn_level_down;                      {down into QUOTED_STRING_CHARS syntax}
...
1: begin                               {chunk of raw characters}
      syn_get_tag_string (str_h, token); {get raw string characters}
...
syn_tag_end_k: begin                   {done reading quoted string}
      syn_level_up;                    {back up from QUOTED_STRING_CHARS syntax}
      return;
      end;
...
begin
  syn_level_down;                      {down into LIT_STRING syntax}
...
2: begin                               {ASCII character value in ()}
  ...
  if val.dtype <> sst_dtype_int_k then begin {expression was not an integer ?}
    syn_error (str_h, 'sst_pas_read', 'exp_not_int', nil, 0);
    end;
  string_append1 (str, chr(val.int_val)); {append this character to output string}
  goto str_loop;                       {back for next part of this string}
  end;

syn_tag_end_k: begin                   {hit end of string, STR all set}
  end;
...
otherwise
  ...
  syn_level_up;                        {up from LIT_STRING syntax}
  end;
