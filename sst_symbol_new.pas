...
*   from one of the SYO_GET_TAG routines.  SYMBOL_P is returned pointing to
...
*
*     SYO_CHARCASE_DOWN_K - Force symbol name to all lower case.
*     SYO_CHARCASE_UP_K   - Force symbol name to all upper case.
*     SYO_CHARCASE_ASIS_K - Do not alter name.
}
...
begin
  ...
  syo_get_tag_string (name_h, name);   {get actual symbol name}
  case charcase of
syo_charcase_down_k: string_downcase(name);
syo_charcase_up_k: string_upcase(name);
syo_charcase_asis_k: ;
otherwise
