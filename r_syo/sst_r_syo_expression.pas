...
1: begin
  sst_r_syo_jtargets_make (            {make jump targets for nested routine}
    jtarg,                             {template jump targets}
    jt,                                {output jump targets}
    lab_fall_k,                        {YES action}
    lab_same_k,                        {NO action}
    lab_same_k);                       {ERR action}
  sst_r_syo_item (jt, sym_mflag);      {process ITEM syntax}
  sst_r_syo_jtargets_done (jt);        {define any implicit labels}

  syo_get_tag_msg (                    {get tag for nested expression}
    tag, str_h, 'sst_syo_read', 'syerr_define', nil, 0);
  sst_r_syo_expression (jtarg, sym_mflag); {process EXPRESSION after ITEM}
  end;
{
**************************************
*
...
3: begin
  sst_r_syo_item (jtarg, sym_mflag);
  end;
{
**************************************
*
